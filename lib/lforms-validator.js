...
/**
 * Constructor definition with input stream and options.
 *
 * @param {Object|String} input - A readable input stream object, a file name,
 *   or json input string. If json input string, caller has to set options.isInputString to true.
 * @param {Object} options - Validator options
 *   source: Name of the source. Ex: a file name/url or any identifying string.
 *   isInputString: Set true if the input is a json string. Default is false.
 *   verbose: Set true of wanted verbose output. Default is false.
 *   outputStream: Set an output stream to write the output. Default is standard out.
 *
 * @constructor
 */
...
lodash.extend(LFormsValidator.prototype,  {
  ...
  validate: function () {
    ...
    var self = this;
    var parser = oboe(input)
      .node({
        'obxTableColumns.*': self.validateObxTableColumn.bind(self),
  ...
  /**
   * Oboe call back handler.
   *
   * Validate datatype.
   *
   * @param {Object} param - Data type node.
   ...
   * Oboe call back handler.
   *
   * Validate ObxTableColumn
   *
   * @param {Object} param - ObxTableColumn instance
   * @param {Array} path - Ancestral path
   */
  validateObxTableColumn: function (param, path) {
    this.validateDucktype(types.ObxTableColumn, param, path);
  },
